...
import 'package:wheater_app/domain/usecases/get_saved_searches.dart';
import 'package:wheater_app/domain/usecases/get_weather_for_city.dart';
import 'package:wheater_app/domain/usecases/get_weather_for_current_location.dart';
import 'package:wheater_app/presentation/providers/weather_state.dart';
...
class WeatherNotifier extends StateNotifier<WeatherState> {
  ...
  Future<void> fetchWeatherForCity(String cityName) async {
    ...
    final result = await _getWeatherForCity(cityName);
    result.fold(
  ...
  Future<void> fetchWeatherForCurrentLocation() async {
    ...
    try {
      ...
      final result = await _getWeatherForCurrentLocation(position.latitude, position.longitude);
      result.fold(
        (failure) => state = WeatherState.error(failure.message),
        (weather) => state = WeatherState.loaded(weather: weather),
      );
     ...
     _ref.read(savedSearchesNotifierProvider.notifier).fetchSavedSearches();
      ...
      state = WeatherState.error('Failed to get location or weather: ${e.toString()}');
    }
  }
}
...
final savedSearchesNotifierProvider = StateNotifierProvider<SavedSearchesNotifier, SavedSearchesState>((ref) {
...
class SavedSearchesNotifier extends StateNotifier<SavedSearchesState> {
  ...
  SavedSearchesNotifier(this._getSavedSearches) : super(const SavedSearchesState.initial()) {
